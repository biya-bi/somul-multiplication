function updateMultiplication() {
	$.ajax({
		url : "http://localhost:9080/multiplications/random"
	}).then(function(data) {
		// Cleans the form
...
function updateStats(alias) {
	$.ajax({
		url : "http://localhost:9080/results?alias=" + alias,
	}).then(
			function(data) {
				$('#stats-body').empty();
													...
													success : function(result) {
														if (result.correct) {
															...
															$('.result-message')
																	.empty()
																	.append(
																			"Oops that's not correct! But keep trying!");
														}
													}
												});
										updateMultiplication();
